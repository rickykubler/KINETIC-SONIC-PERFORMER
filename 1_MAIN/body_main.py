...
def draw_hsv(flow):
    ...
    hsv = np.zeros(
        (h, w, 3), np.uint8
    )  # Creates an image filled with zero intensities with the same dimensions as the frame (hxwx3 dimension)
    hsv[..., 0] = ang * (
        180 / np.pi / 2
    )  # Sets image color according to the optical flow direction
...
with mp_holistic.Holistic(
    model_complexity=1, min_detection_confidence=0.0, min_tracking_confidence=0.0
) as holistic:
    while cap.isOpened():
        ...
        distance_thumb = (
            (center_x - right_thumb_x) ** 2 + (center_y - right_thumb_y) ** 2
        ) ** 0.5
        # distance_index=((center_x - index_x)**2 + (center_y - index_y)**2)**0.5
        distance_middle = (
            (center_x - right_middle_x) ** 2 + (center_y - right_middle_y) ** 2
        ) ** 0.5
        distance_ring = (
            (center_x - right_ring_x) ** 2 + (center_y - right_ring_y) ** 2
        ) ** 0.5
        distance_pinky = (
            (center_x - right_pinky_x) ** 2 + (center_y - right_pinky_y) ** 2
        ) ** 0.5
        ...
        hand_expansion = (
            (right_wrist_x - left_wrist_x) ** 2 + (right_wrist_y - left_wrist_y) ** 2
        ) ** 0.5
