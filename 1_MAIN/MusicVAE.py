...
def send_generate_sequence_lin(temperature=0.01, chord_1="C", std=1):
    ...
    z = (
        temp_z
        + direction / norm_direction * SPEED
        + np.random.normal(0, std, size=[1, Z_SIZE])
    )
    temp_z = z
    ...
    seqs = [
        model.decode(
            length=TOTAL_STEPS,
            z=np.random.normal(size=[1, TOTAL_STEPS]),
            temperature=temperature,
            c_input=chord_encoding(c),
        )[0]
        for c in chords
    ]
...
def send_generate_sequence(temperature=0.01, chord_1="C"):
    ...
    if nsteps == 0:
        ...
        z = np.array([slerp(start, target, t) for t in np.linspace(0, 1, nsteps)])
        index_iter = 0
