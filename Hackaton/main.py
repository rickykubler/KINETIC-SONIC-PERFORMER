...
def countFingers(image, results, draw=True, display=True):
    ...
    for hand_index, hand_info in enumerate(results.multi_handedness):
        ...
        if hand_label == "Left":
            coord_x_left = (
                hand_landmarks.landmark[middle_finger_tip].x
                - hand_landmarks.landmark[wrist].x
            )
            coord_y_left = (
                hand_landmarks.landmark[middle_finger_tip].y
...
def set_client():
    ...
    )  # if in the same machine
    parser.add_argument(
        "--port", type=int, default=7500, help="The port the OSC server is listening on"
    )
...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        image, fingers_statuses, count, OPEN_RIGHT, OPEN_LEFT, left_hand_angle = (
            countFingers(image, results, display=False, draw=False)
        )

        for hand_index, handLms in enumerate(
